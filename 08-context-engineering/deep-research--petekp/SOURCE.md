...
- Local skill path: `08-context-engineering/deep-research--petekp`
- License: MIT (Copyright (c) 2024 Pete Petrash)
- Imported: 2026-09-05

## Local folder name

Upstream slug is `deep-research`. The local folder is `deep-research--petekp` because `08-context-engineering/deep-research` already exists from a different origin. Frontmatter `name:` was rewritten to match the folder so the validator resolves the catalog entry. The skill body is otherwise unmodified.
