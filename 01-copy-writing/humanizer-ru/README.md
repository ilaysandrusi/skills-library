...
1. Откройте страницу **Releases** этого репозитория, выберите свежий выпуск и скачайте приложенный к нему архив `humanizer-ru.zip`. Это собранный архив скилла: его состав перечислен в шаге 0 и в точности совпадает с этим списком. `Source code (zip)`, который GitHub добавляет к каждому выпуску, — полное дерево репозитория вместе с `.github/`, `research/` и `tests/`; он нужен только тем, кто собирается запускать валидаторы.
...
Полная запись доказательств — неизменяемая ссылка на источник, дата обращения, дословный образец, класс доказательства и fixture — есть у 37 из 39 маркеров. Остальные закрыты только фикстурами. Валидатор печатает это число как есть, а не округляет его до удобного.
