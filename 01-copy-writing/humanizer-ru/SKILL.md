...
# Humanizer-ru — очеловечивание текста (v3.11.0)
...
Жанр? — код / конфиг → отказ
        ...
        — художка / поэзия → не применять правило трёх (#13), #16 длинное тире, см. false-positives.md
...
Пользователь сообщил дату создания текста?
   ...
   ↓
Прогнать regex по chatbot-artifacts.md (текст в файле; если доступен Grep — ищите готовыми подстроками из раздела «Детерминированная проверка»)
  ↓
...
правка запрошена → глубокая перезапись по rewrite-guide.md
   ↓
Если есть ссылки на источники → прогнать source-fabrication.md (офлайн: флаги «требует проверки»)
   ↓
...
| `references/language-patterns.md` | Языковые паттерны #10–15 и русские расширения #15a–15f: деепричастия, каскад смягчений, связки-переходы и обороты-затычки, смена стиля, семантический сдвиг через английское поле, отсутствие идиоматики | Всегда при анализе связного текста |
...
| `references/rewrite-guide.md` | Процедура выборочной правки: порядок по критичности, сохранение голоса и жанра, запрет на дописывание фактов, глубокая перезапись целиком машинного текста, невидимая маркировка моделей, жанровые режимы подачи, петля самопроверки, два примера | Когда правка запрошена явно |
...
- ✓ Ссылки на источники прогнаны через офлайн-проверки `source-fabrication.md`, спорные помечены как требующие проверки?
...
Этот файл и `references/*` построены однотипно: «Проблема → Маркер → Что делать → Граница ложного срабатывания → До/После». Симметрия справочника — навигационное удобство, не сигнал генерации. Не путать с #13 из `language-patterns.md`: там о симметрии в авторских текстах.
