...
- Upstream path: `skills/claude-code-audit`
- Local skill path: `09-anthropic-tools/claude-code-audit`
...
`scripts/inventory.py` defaults to reading `~/.claude/projects` (the user's own local transcripts) with no network. Documented, not auto-invoked, same class as planning-with-files before 3.16.0.
