#!/usr/bin/env python3
"""
Canonical URL safety module for seo-skills.
...
seo-skills scripts are intentionally single-threaded; parallelism is
...
# This is a deliberate choice: seo-skills scripts run one URL fetch at a
...
def _pin_dns(hostname: str, pinned_ip: str, port: int) -> Iterator[None]:
    ...
    if not _dns_patch_lock.acquire(blocking=False):
        raise URLSafetyError(
            ...
            "seo-skills url_safety is not thread-safe by design."
        )
...
def _cli() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Validate a URL against seo-skills's SSRF policy."
    )
