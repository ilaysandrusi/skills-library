...
- **direct** — Clear scope and a known approach, ready to apply via `/turbo-implement`.
...
- **Lane shape** — Each lane batches a cluster, just in different shapes. Direct groups clear-scope fixes into one `/turbo-implement` run. Investigate dispatches `/investigate` per symptom, then shares one `/turbo-implement` for the concluded fixes. Plan hands a cohesive cluster to `/turboplan`, which routes it to a plan file.
