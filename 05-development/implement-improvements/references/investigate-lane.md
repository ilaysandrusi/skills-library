...
Diagnose each working-set entry via `/investigate`, then apply the concluded fixes via `/turbo-implement`.
...
2. Run `/turbo-implement` skill for the concluded fixes
...
## Phase 2: Run `/turbo-implement` Skill for the Concluded Fixes

In the turn that invokes `/turbo-implement`, write out each investigation's concluded fix as an explicit bullet: summary + files + change description. Being explicit matters here because `/investigate`'s earlier output has likely displaced continuation context, so `/turbo-implement` needs a fresh, self-contained description.

Then run the `/turbo-implement` skill.

Then use the TaskList tool and proceed to any remaining task.

## Rules

- Run `/finalize` only once (inside `/turbo-implement` in Phase 2), not once per investigation.
