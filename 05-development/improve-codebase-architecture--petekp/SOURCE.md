...
- Upstream path: `skills/improve-codebase-architecture`
- Local skill path: `05-development/improve-codebase-architecture--petekp`
- License: MIT (Copyright (c) 2024 Pete Petrash)
- Imported: 2026-09-05

## Local folder name

Upstream slug is `improve-codebase-architecture`. The local folder is `improve-codebase-architecture--petekp` because `05-development/improve-codebase-architecture` already exists from a different origin. Frontmatter `name:` was rewritten to match the folder so the validator resolves the catalog entry. The skill body is otherwise unmodified.
