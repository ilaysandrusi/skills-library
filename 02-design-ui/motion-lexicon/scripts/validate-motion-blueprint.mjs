/* Generated by scripts/build-motion-blueprint-validator.mjs. Edit scripts/validate-motion-blueprint-source.mjs, then rebuild. Third-party notices: skills/motion-lexicon/THIRD_PARTY_NOTICES.md. */
...
// scripts/validate-motion-blueprint-source.mjs
var import__ = __toESM(require__(), 1);
import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";
var scriptDirectory = path.dirname(fileURLToPath(import.meta.url));
var skillDirectory = path.resolve(scriptDirectory, "..");
var schemaPath = path.join(skillDirectory, "assets", "motion-blueprint.schema.json");
var defaultBlueprintPath = path.join(skillDirectory, "assets", "example-motion-blueprint.json");
var providedPath = process.argv[2];
var blueprintPath = providedPath ? path.resolve(process.cwd(), providedPath) : defaultBlueprintPath;
var issues = [];
var addIssue = (message) => issues.push(message);
var readJson = (filePath, label) => {
  try {
...
var decodeJsonPointerSegment = (segment) => segment.replace(/~1/g, "/").replace(/~0/g, "~");
var formatInstancePath = (instancePath) => {
  if (!instancePath) return "blueprint";
  return instancePath.split("/").slice(1).map(decodeJsonPointerSegment).reduce(
    (label, segment) => Number.isInteger(Number(segment)) && String(Number(segment)) === segment ? `${label}[${segment}]` : `${label}.${segment}`,
    "blueprint"
  );
};
var formatSchemaError = (error) => {
...
var schemaResult = readJson(schemaPath, "Motion Blueprint schema");
var blueprintResult = readJson(blueprintPath, "Blueprint");
if (schemaResult.ok && blueprintResult.ok) {
  try {
    const validator = new import__.default({ allErrors: true }).compile(schemaResult.value);
