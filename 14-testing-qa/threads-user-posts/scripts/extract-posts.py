...
def main():
    ...
    (function() {
      try {
        ...
        const mediaIdx = html.indexOf('"mediaData":{"edges"');
        if (mediaIdx < 0) {
          return JSON.stringify({error: true, message: 'mediaData not found in SSR HTML — ensure the user profile page is fully loaded'});
        }
        let depth = 0;
        const start = html.indexOf('{', mediaIdx + '"mediaData":'.length);
        if (start < 0) return JSON.stringify({error: true, message: 'mediaData object start brace not found'});
        let pos = start;
        ...
        let media;
        try {
          media = JSON.parse(html.slice(start, pos + 1));
        } catch(pe) {
          return JSON.stringify({error: true, message: 'JSON parse failed: ' + pe.message});
        }
        const edges = media.edges || [];
        if (!edges.length) return JSON.stringify({error: true, message: 'No posts found in mediaData'});
        const posts = [];
        for (const edge of edges) {
          const items = edge.node && edge.node.thread_items;
          if (!items || !items.length) continue;
          const post = items[0].post;  // primary post in thread
          if (!post) continue;
          const u = post.user || {};
          ...
          posts.push({
            ...
            reshare_count: info.reshare_count || 0,
        ...
        return JSON.stringify({
          posts: posts,
          count: posts.length,
          page_info: media.page_info || null
        });
      } catch(e) {
