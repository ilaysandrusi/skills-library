param(
    ...
    function Write-GateArtifacts {
        ...
        $jsonPath = Join-Path $dir 'vibe-browserops-scorecard-gate.json'
        $mdPath = Join-Path $dir 'vibe-browserops-scorecard-gate.md'
        Write-VgoUtf8NoBomText -Path $jsonPath -Content ($Artifact | ConvertTo-Json -Depth 100)

        $lines = @(
            '# VCO BrowserOps Scorecard Gate',
            '',
    ...
    $artifact = [pscustomobject]@{
        gate = 'vibe-browserops-scorecard-gate'
        repo_root = $repoRoot
