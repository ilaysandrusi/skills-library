...
7. **Run the value-vs-count reconciliation** — cross-check that total value the platform would receive over a recent period tracks the net profit the business actually booked. If the platform's summed conversion value is 3× the real contribution, tROAS is optimizing to a phantom number — flag it. This is a *sanity check on the value model*, not the cross-platform order-ID de-dup, which stays in [attribution-reconciler](../attribution-reconciler/SKILL.md); if the live totals won't reconcile across platforms, route there.
