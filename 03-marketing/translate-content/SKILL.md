...
6. **Score translation quality**: Run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/language-router.py" --action score --source "{source}" --target "{target}" --original "{source_content}" --translated "{translated_content}"` to assess quality across dimensions:
...
9. **Run brand voice check**: Execute `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/brand-voice-scorer.py" --brand {slug} --text "{translated_content}"` to assess whether the translated content maintains brand voice characteristics. Flag any voice drift with specific examples and suggestions.
