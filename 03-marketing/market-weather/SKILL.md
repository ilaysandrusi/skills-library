...
2. **Generate weather report**: Run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/macro-signal-tracker.py" --brand {slug} --action weather-report` to pull the latest recorded signal data across all five dimensions. (The script reads the brand's industry from its profile; the time horizon is applied when you weight and phrase the recommendations, not as a script flag — there are no `--industry`/`--horizon` flags.) If specific concerns were provided, prioritize them in the analysis.
