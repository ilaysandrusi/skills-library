...
5. **Calculate sample size per variant and minimum budget**: Compute the required conversions per variant with `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/sample-size-calculator.py" --baseline-rate {rate} --mde {mde} --mde-type relative --significance 0.95 --power 0.80` (a "10-20% relative lift" is `--mde 0.10`-`0.20` with `--mde-type relative`; use `--mde-type absolute` if the target is stated in percentage points — the two differ by ~40× at a 5% baseline). Translate sample size into minimum budget per test based on current CPM/CPC rates.
