...
2. **Check campaign history**: Run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/campaign-tracker.py" --brand {slug} --action list-campaigns` to pull historical campaign data for trend comparison and period-over-period analysis.
3. **Run ROI calculator**: Execute `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/roi-calculator.py"` with spend, revenue, and conversion data to compute channel-level and blended metrics.
