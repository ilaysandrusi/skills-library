...
2. **Query the intelligence graph**: Execute `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/intelligence-graph.py" --brand {slug} --action query-relevant --context '{context_json}' --min-confidence {threshold}`. The query matches against all indexed conditions — channel, audience, objective, campaign type — and also performs semantic matching for freeform situation descriptions. `--min-confidence` applies the confidence-threshold filter; apply the time-range preference during ranking (step 3), not as a query flag.
