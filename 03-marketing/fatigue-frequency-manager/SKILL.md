...
2. **Build the trend, not a snapshot.** Read frequency, CTR, and CVR as a slope from baseline to now. Snapshot to the ledger so the delta is computed, not eyeballed: `python3 "${CLAUDE_PLUGIN_ROOT}/../../scripts/aaron-marketing/connectors/ledger.py" record <ad-set> --source paid --data '{"frequency": ..., "ctr": ..., "cvr": ..., "reach": ...}'`, then `ledger.py trend <ad-set> --source paid --field ctr` (repeat for `frequency`, `cvr`, `reach`).
...
Ask "Save these results?" If yes, write to `memory/ad/fatigue-frequency-manager/` using `YYYY-MM-DD-<ad-set>-fatigue.md` — see [Skill Contract](../../references/aaron-marketing/skill-contract.md) §Save Results Template. This skill asks before writing memory and hands off veto-like measurement risks to `ad-account-auditor` rather than marking a veto itself.

## Reference Materials

- [ROAS Benchmark](../../references/aaron-marketing/roas-benchmark.md) — the paid-ads scoring framework; this skill works the **S** (CTR/CVR/frequency decay under spend-efficiency) and **R** (return-protection) levers; the Return vetoes R1/R2 govern whether a CVR-based read is trustworthy. Only `ad-account-auditor` computes the RQS or runs vetoes.
- [Measurement & Attribution Protocol](../../references/aaron-marketing/measurement-protocol.md) — baseline windows, conversion-lag handling, and frequency-ceiling guidance by objective.
- [scripts/connectors/README.md](../../scripts/aaron-marketing/connectors/README.md) — `ledger.py` record / trend reference for the decay slope.
...
Visited-set and `max-depth: 3` termination rules apply per [Skill Contract](../../references/aaron-marketing/skill-contract.md); if the recommended target was already run this chain, STOP and report chain-complete.
