...
4. **Optimize posting times — the brand's data first**: If the brand has post-level engagement history (exported from platform analytics: timestamps + engagement per post), write it to a JSON file and run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/posting-time-analyzer.py" --platform {platform} --history {file}` — first-party windows come back ranked with sample sizes and are the only recommendations that can earn high confidence. Without history, run it with `--platform {platform} --industry {industry} --audience-type {b2b|b2c|mixed}` for the DATED population baseline (`baseline_as_of` + capped-at-medium ceiling in the output; a stale baseline refuses with exit 3 — refresh it or supply history). Surface the output's `algorithm_note` — on interest-ranked feeds, content strength dominates timing. Factor in audience timezone distribution.
