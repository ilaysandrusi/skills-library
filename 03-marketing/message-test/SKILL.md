...
2. **Load audience panel**: Reference the existing panel by its ID (list available panels with `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/audience-simulator.py" --brand {slug} --action list-panels`), or create a new panel via `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/audience-simulator.py" --brand {slug} --action create-panel --panel-name {name} --segments '[...]'` with CRM data grounding if new segment definitions were provided. Verify the panel has sufficient segment diversity for meaningful cross-segment comparison.
3. **Test each variant against each segment**: Run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/audience-simulator.py" --brand {slug} --action test-message --panel-id {id} --variants '[...]'` for the variant set. Score each variant on every evaluation criterion (resonance, clarity, credibility, urgency, differentiation) from the perspective of each segment's behavioral profile. Generate predicted response sentiment, key reactions, and specific objections for each combination.
