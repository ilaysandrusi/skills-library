...
- **Reads**: the flow type or lifecycle goal, the trigger event, the audience/segment (from the user or from [list-segment-builder](../list-segment-builder/SKILL.md) when present), an ESP flow/automation export (own data) and current cadence/complaint signals when available, and one SEND profile (`promotional|retention|cold-outbound|newsletter`).
...
- **Done when**: every flow has a trigger, per-step timing, a goal, and explicit branch/exit conditions; a global send cap + quiet hours + a fatigue guardrail are specified; a re-engagement/sunset path exists for the engagement-decay sub-item; and the SEND **N** score is emitted with the typed profile named.
- **Primary next skill**: [email-creative-builder](../email-creative-builder/SKILL.md) to write each step, or [email-quality-auditor](../email-quality-auditor/SKILL.md) to score the program and enforce N1.
...
**Zero-dependency flow activation (when Resend is the ESP)**: once a flow step's creative is approved, `python3 "${CLAUDE_PLUGIN_ROOT}/scripts/connectors/resend.py" broadcast-create --segment <segment-id> --from … --subject … --html step.html` then `resend.py broadcast-send <id> --at "<ISO 8601>"` schedules the step against its segment (`resend.py segments` lists the ids; one-off timed sends use `resend.py send --scheduled-at …`). Every mutating subcommand is dry-run by default — show the user the previewed request, then re-run with `--live`. Never enroll a suppressed or non-consented subject: the [consent-registry](../consent-registry/SKILL.md) check comes first. See [scripts/connectors/README.md](../../scripts/aaron-marketing/connectors/README.md).
...
- **Primary**: [email-creative-builder](../email-creative-builder/SKILL.md) — fill each step of the designed flows with subject, preheader, body, and CTA.
- **If the flow map is ready for the gate**: [email-quality-auditor](../email-quality-auditor/SKILL.md) — score the profile-weighted EQS and enforce N1 (unsubscribe integrity) and the other vetoes.
