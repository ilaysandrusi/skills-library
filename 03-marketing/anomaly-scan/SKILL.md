...
3. **Load historical baselines**: Execute `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/performance-monitor.py" --brand {slug} --action get-baseline`
...
4. **Run anomaly detection**: Execute `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/performance-monitor.py" --brand {slug} --action detect-anomalies --data '{...current-period metrics...}'`
...
5. **Cross-reference with recent executions**: Check execution history via
   `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/execution-tracker.py" --brand {slug} --action get-history --limit 14`
