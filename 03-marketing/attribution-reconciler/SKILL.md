...
- **Primary next skill**: [roi-calculator](../roi-calculator/SKILL.md).
...
7. **Hand the ratios to roi-calculator.** This workbook produces clean, de-duped, normalized conversion and order counts. It does **not** compute ROAS, CPA, ROI %, or EMV — pass the reconciled counts to [roi-calculator](../roi-calculator/SKILL.md) for all ratio math. State which counts to feed it (de-duped real orders, by platform).
...
**Primary**: [roi-calculator](../roi-calculator/SKILL.md) — turn the de-duped, normalized counts into ROAS/CPA/ROI.

Alternates: [report-generator](../report-generator/SKILL.md) once the ratios are in, or [ad-account-auditor](../ad-account-auditor/SKILL.md) if the reconciliation surfaces a point-in-time integrity problem (broken tracking, systemic double-count) that needs the gate.
