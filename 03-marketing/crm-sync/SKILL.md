...
2. **Check connected CRM status**: Run `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/crm-sync.py" --brand {slug} --action get-crm-status` to verify platform connection, API credentials, rate limit headroom, and available objects. If no CRM is connected, guide the user to configure their CRM MCP integration and provide platform-specific setup instructions.
