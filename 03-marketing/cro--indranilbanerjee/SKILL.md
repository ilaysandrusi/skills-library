...
4. **Calculate requirements** -- Determine sample size with `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/sample-size-calculator.py" --baseline-rate {rate} --mde {mde} --mde-type absolute --significance 0.95 --power 0.80` (pass `--mde-type relative` if the MDE is a relative lift — the two differ by roughly two orders of magnitude, ~200×, at a 5% baseline). Estimate test duration based on daily traffic.
