...
3. **Check for related learnings**: Query the intelligence graph via `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/intelligence-graph.py" --brand {slug} --action query-relevant --context '{"channel":"...","audience":"...","objective":"..."}'` using the learning's context conditions. Search for existing learnings that overlap in channel, audience, and objective to detect duplicates, supporting evidence, or contradictions.
...
5. **Save the learning**: If the learning is new or the user confirmed the update, save via `python "${CLAUDE_PLUGIN_ROOT}/../../scripts/digital-marketing-pro/intelligence-graph.py" --brand {slug} --action save-learning --agent "{source}" --insight "{insight text}" --conditions '{"channel":"...","audience":"...","objective":"..."}' --confidence {score}` (add `--evidence "{supporting evidence}"` if provided) with the full structured record. The learning is indexed by all context conditions for multi-dimensional retrieval.
