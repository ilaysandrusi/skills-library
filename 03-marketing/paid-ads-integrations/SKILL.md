...
1. Inspect the available tool list and use the Google or Meta shared preamble when that surface is requested. For X Ads and LinkedIn Ads, resolve the platform-specific connector and require a harmless account/setup read before claiming access.
...
| X Ads | `https://notfair.co/api/mcp/x_ads`; resolve `~~x-ads` | Connect/re-authenticate, then use `/notfair:paid-ads-x` |
| LinkedIn Ads | `https://notfair.co/api/mcp/linkedin_ads`; resolve `~~linkedin-ads` | Connect/re-authenticate, then use `/notfair:paid-ads-linkedin` |
