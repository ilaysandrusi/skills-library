...
7. **Run the stand-down.** All-clear when velocity holds under 1.5x baseline for 24h (Estimated defaults, user-tuned). Then: reconcile pause markers via [channel-registry](../channel-registry/SKILL.md), re-run [social-quality-auditor](../social-quality-auditor/SKILL.md) pre-publish mode on the still-queued content (pre-crisis drafts may now be tone-wrong or claim-stale) before un-pausing, and append the dated un-pause markers.
