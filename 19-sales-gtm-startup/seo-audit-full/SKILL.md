...
4. **Delegate to subagents** (if available, otherwise run inline sequentially):
   ...
   - `seo-google` -- CWV field data (CrUX), URL indexation (GSC), organic traffic (GA4) (spawn when Google API credentials detected via `python ../../scripts/infrasity-dev-gtm/google_auth.py --check`)
   - `seo-backlinks` -- Backlink profile data: DA/PA, referring domains, anchor text, toxic links (spawn when Moz or Bing API credentials detected via `python ../../scripts/infrasity-dev-gtm/backlinks_auth.py --check`, or always include Common Crawl domain-level metrics)
   ...
   - `seo-drift` -- Drift analysis: compare against stored baseline (spawn when drift baseline exists for the URL via `python ../../scripts/infrasity-dev-gtm/drift_history.py <url>`)
