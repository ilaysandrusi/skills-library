...
This contract answers a failure pattern from an earlier cycle: skills had reviewers, but the reviewer ran as advisory and the writer presented sloppy drafts anyway. The fix is infrastructure, not effort. Same shape as `scripts/lint_prose.py` and `tests/test_installer_sync.py`.
