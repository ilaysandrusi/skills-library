...
claude-blog adopts the FLOW evidence-led model. The 6 Pillars stay as-is; they become the operational expression of FLOW's principles. Mapping:
...
| Delivery contract | All 5 gates pass | Blocked drafts iterate up to 3x; see `references/blog-delivery-contract.md` |
...
Load on-demand as needed (21 references):
...
- `references/research-quality.md`: 5-dim research rubric, pre-flight trap classes, cross-source clustering, freshness floors
- `references/synthesis-contract.md`: 6 LAWs for research-synthesis output
- `references/blog-delivery-contract.md`: 5-gate enforcement between content generation and user delivery
...
| `blog-cluster` | Semantic topic-cluster planning + execution (hub-and-spoke architecture) |
| `blog-flow` | FLOW framework prompts: find, optimize, win, prompts index, sync |
| `blog-multilingual` | One-command international publishing: write + translate + localize + hreflang |
| `blog-translate` | SEO-optimized translation with format preservation (markdown, MDX, frontmatter, schema) |
...
| `blog-reviewer` | Quality assessment: runs 100-point scoring, AI content detection (no Bash) |
| `blog-translator` | Multilingual translation specialist; format preservation across markdown/MDX/HTML/frontmatter/schema (no Bash) |
...
6.5. **Delivery Contract Enforcement**: Run the 5-gate preflight per `references/blog-delivery-contract.md`. Generate hero via `scripts/generate_hero.py`. Render `.md`/`.html`/`.pdf` via `scripts/blog_render.py`. Run `scripts/blog_preflight.py --draft <folder> --strict`. Check the `BLOCKING:` line in `<folder>/review.md` written by Step 6. If any gate blocks: loop back to Step 4 with the failure diagnostic; max 3 iterations; on the 3rd failure, STOP and present the diagnostic instead of the draft. The user is NEVER the first reviewer; the gates are.
