...
This sub-skill uses WebSearch with platform-targeted site operators. No API keys required.
...
`blog-discourse` is API-free, using WebSearch with platform-targeted site operators across Reddit, X, YouTube, HN, dev.to, Medium and similar platforms. The discourse-research discipline (pre-flight trap classes, named-entity decomposition, cross-source clustering, freshness floors, synthesis-contract LAWs) is applied as prompt discipline.
